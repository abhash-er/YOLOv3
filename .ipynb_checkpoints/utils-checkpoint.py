...
#convert middle(x,y) to (top_left,top_right)

def output_boxes(inputs,model_size,max_output_size,max_output_size_per_class,iou_threshold,confidence_threshold):
    
    centre_x, centre_y, width, height, confidence, classes = tf.split(inputs,[1,1,1,1,1,-1], axis = -1)
    
    top_left_x = centre_x - width / 2.0
    top_left_y = centre_y - height / 2.0
    
    bottom_right_x = centre_x + width / 2.0
    bottom_right_y = centre_y + height / 2.0
